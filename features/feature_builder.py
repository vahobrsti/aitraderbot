...
def build_features_and_labels_from_raw(
    ...
    # ---------- 0) Bitcoin Halving Cycle Features ----------
    # Bitcoin halvings occur ~every 4 years (~1460 days). The market behaves
    # differently at different phases of the cycle. This gives the model
    # context to interpret indicator readings correctly.
    
    # Historical halving dates (block reward halved)
    HALVING_DATES = [
    ...
    # Phase buckets (one-hot) - different market behavior in each phase
    # Phase 0-6m:   Post-halving accumulation (often choppy/consolidation)
    # Phase 6-12m:  Early bull (price discovery begins)
    # Phase 12-18m: Peak bull (euphoria, blow-off top zone)
    # Phase 18-30m: Bear market (correction, capitulation)
    # Phase 30-48m: Accumulation/recovery (smart money accumulates)
    ...
    # Smooth seasonality (sin/cos encoding for cyclical nature)
    # This captures smooth transitions between phases
